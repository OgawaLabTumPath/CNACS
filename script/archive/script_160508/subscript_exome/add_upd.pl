...
while (<BAF_ALL>) {
	...
	next if ( $baf eq 'NA' );
	if ( ( $baf < 0.05 ) && ( ! defined $cna_region{$key} ) ) {
		...
		if ( $short_flag == 1 ) {
			if ( ( $homo_num >= 100 ) && ( $chr ne 'chr13' ) && ( $chr ne 'chr14' ) && ( $chr ne 'chr15' ) && ( $chr ne 'chr21' ) && ( $chr ne 'chr22' ) ) {
				$upd_region{$cur_start} = $pre_pos;
				$region2baf_all{$cur_start} = join(",", @cur_bafs);
			}
			$short_flag = 0;
...
foreach my $tmp_region ( keys %upd_region ) {
	...
	$pos2info{$region} = '"' . $id . '"' . "\t" . $region . "\t" . $end . "\t" . $region2num_all{$tmp_region} . "\t" . $region2ave{$tmp_region} . "\t" . $region2baf{$tmp_region};
}
